...
function isOrderOverdue(timestamp) {
  // 請實作此函式
  return dayjs().diff(dayjs.unix(timestamp), 'day') > 7;
}
...
function getThisWeekOrders(orders) {
  ...
  return orders.filter(order => {
    const isBeforeMatch = dayjs().startOf('week').isBefore(dayjs.unix(order.createdAt));
    const isAfterMatch = dayjs().endOf('week').isAfter(dayjs.unix(order.createdAt));

    return isBeforeMatch && isAfterMatch;
  });
}
...
async function addToCartWithAxios(productId, quantity) {
  ...
  const response = await axios.post(`${BASE_URL}/api/livejs/v1/customer/${API_PATH}/carts`, {data});
...
1. JSON 解析：fetch 要手動處理 ; axios 會自動解析。

2. Content-Type：fetch 要手動設定 ; axios 會自動設定。

3. 錯誤處理：fetch 4xx/5xx 不會拋錯 ; axios 4xx/5xx 會自動拋錯。
...
const OrderService = {
  ...
  formatOrders(orders) {
    // 請實作此函式
    return orders.map(order => ({...order, formattedDate: dayjs.unix(order.createdAt).format('YYYY-MM-DD')}));
  },
